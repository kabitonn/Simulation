...
class MyTopo(Topo):
    def __init__(self):
        ...
        for i in range(len(hosts)):
            self.addLink(hosts[i],switches[i/(Host_Num/Switch_Num)])
        for i in range(len(switches)):
            self.addLink(switches[i],switches[(i+1) % Switch_Num])
...
def configNet():
    ...
    [net.get(hosts[i]).setIP('192.168.5.'+str(i+1),21)for i in range(len(hosts))]
    [net.get(hosts[i]).cmd('route add -net default dev ' + hosts[i] + '-eth0')for i in range(len(hosts))]
